...
import { buildP8IntegrityBaseline, validateP8IntegrityBaseline } from "./p8-integrity-baseline.mjs";
...
const p8IntegrityBaseline = await readJson(resolve(bedrockRoot, "data", "p8-integrity-baseline.json"));
...
const p8IntegrityCoverage = validateP8IntegrityBaseline(p8IntegrityBaseline);
...
assertFreshGeneratedData("bedrock/data/p8-integrity-baseline.json", p8IntegrityBaseline, await buildP8IntegrityBaseline({ bedrockRoot }));
...
const nativeRecipeEntries = await Promise.all((await jsonFiles(nativeRecipeDirectory))
	.map(async file => [relative(nativeRecipeDirectory, file).replaceAll("\\", "/"), await readFile(file, "utf8")]));
