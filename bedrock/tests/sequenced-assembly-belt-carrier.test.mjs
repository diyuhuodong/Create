...
test("sequenced Belt carrier owns its session and item in one persisted transport record", () => {
	...
	const destination = network.createDepot({ dimensionId: "minecraft:overworld", location: { x: 8, y: 64, z: 0 }, size: 2 });
	...
	assert.equal(bridge.beginAtStation({ stationType: "press", transportId: transport.id }).accepted, true);
	...
	assert.equal(resumed.apply({ stationId: "press:one", stationType: "press", transportId: transport.id }).applied, true);
	assert.deepEqual(resumed.apply({ stationId: "press:one", stationType: "press", transportId: transport.id }), { applied: false, reason: "station_already_applied" });
...
test("sequenced Belt carrier splits a stack into an active item and durable trailing escrow", () => {
	...
	advance(network, () => network.beltTransports().length === 1 && !network.diagnostics().waitingForCommit);
	const bridge = new SequencedAssemblyBeltCarrier({ network, recipes: RECIPES });
	const transportId = network.beltTransports()[0].id;
	assert.equal(bridge.begin({ recipeId: RECIPES[0].id, transportId }).accepted, true);
	assert.deepEqual(network.beltCarrier(transportId).queuedItems, [{ count: 1, typeId: "minecraft:iron_ingot" }]);
	assert.equal(bridge.apply({ stationType: "press", transportId }).applied, true);
	assert.equal(bridge.release({ transportId }).released, true);
	advance(network, () => !network.beltCarrier(transportId), 300);
	const delivered = network.snapshot().find(record => record.kind === "depot" && record.port.id === destination).port.slots.filter(Boolean)
	assert.deepEqual(delivered, [
		{ count: 1, typeId: "createbedrock:finished_test" },
		{ count: 1, typeId: "minecraft:iron_ingot" }
	]);
});
