...
test("migration ledger covers every Java registration and records reviewed mappings only through overrides", async () => {
	...
	assert.deepEqual(validateMigrationOverrides(overrides, catalog), { entries: 9 });
	...
	assert.equal(ledger.registrationEntries.filter(entry => entry.status === "unclassified").length, 872);
