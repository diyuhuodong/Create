...
**状态：** S3-14 已升级到 1.26.0 非实验原生红石基线；29 项红石条目的 R0–R5 代码已完成，逐设备状态为 `implementation_complete_pending_static_validation`。静态回归尚未执行，因此矩阵正式状态仍为 `implementation_in_progress`，不是 `static_verified`。S3-15 已完成平台验收账本、性能基线和静态防伪校验，但 Windows、测试 Realm 与 PS 的物理验收仍全部待执行。
...
| S3-14 | 1.26.0 原生红石基线与 29 项设备实现。使用 non-experimental consumer/producer、事件输入和安全轮询回退。 | S3-6 的稳定输入总线。 | 代码已完成；静态契约逐项校验运行时符号、资源、状态和测试路径。执行回归后才可将矩阵改为 `static_verified`。 |
| S3-15 | 平台验收与性能收敛。 | S3-8 至 S3-14 的代码范围完成。 | 已交付可校验账本和性能基线；Windows 本地世界、测试 Realm、PS 仍需分别完成烟雾测试、双人并发和 30 分钟压力记录。 |

## 工作量边界

当前未达到 `static_verified` 的条目按域分布为：内容 116、动力 33、物流 26、流体 18、加工 8、红石 29。红石的 29 条已完成 R0–R5 代码，等待静态回归后再正式转换状态；升级版本或静态文件存在均不等同于 Windows、Realm 或 PS 通过。
...
7. 已完成：S3-14 将 manifest、行为包格式与 Script API 升至 1.26.0 / 2.5.0 非实验原生红石基线。六类既有输入接入 native consumer 事件并保留 fail-closed 轮询回退；29 条输出/显示/计时记录均具有 producer/consumer 声明、资源、状态运行时和契约测试。R0–R5 后续已完成六频道 Linked Controller、物品栈配置、Lectern 会话、跨 Depot 来源的 Requester intent、网络化可配置调速器、移动机械/电梯 Contact、Display/Nixie 与双粉碎轮控制器的持久化物品加工和皮带交接。其代码状态为 `implementation_complete_pending_static_validation`；静态契约与执行顺序见 [S3-14 静态完成设计](create-bedrock-s3-14-static-completion-design.md)。
