...
P8.2 已完成：329 个旧 `static_verified` 注册项已由 P8.2 重新核验目标定义与运行时入口后收敛为 `implemented`。其余 50 项已明确转交 P8.3：30 项没有旧静态验收记录，20 项缺少可验证的 Bedrock 映射目标，均未被误升级。

P8.3 的注册语义切片已完成：`p8-3-semantic-registration-convergence.json` 为这 50 项提供显式的多对一、组合或虚拟化映射，并同时校验 Bedrock 目标定义、运行时路径及物品获取路径；对应迁移台账已收敛为 `implemented`，由 `p8-semantic-registration-convergence.test.mjs` 覆盖。该切片**不等于 P8.3 全部完成**：Java 的 1,319 个行为来源仍须按本节的逐域行为审计与真实平台验收继续核对。
