...
当前实现已将动力和三类固定加工机迁移到分片存储，并提供 S3-2 的 `ItemPort`、筛选策略、持久化 intent/escrow 状态机与恢复测试。S3-3 已增加 depot 的受控持久化端口：depot 状态、抽取回执和 escrow 日志写入同一提交域，因而可在重启恢复时避免两端重复或吞没物品。`TransportRecord` 已实现 belt 上的唯一物品所有权、速度/反转、满端重试和重启恢复；尚未绑定到可放置的 belt 连接器或渲染。funnel 控制器已接入 `ItemFilter`、锁定状态和同一 escrow 事务，可在受控端口之间筛选转移。chute 控制器复用同一恢复 journal，并已迁移第一版 block 几何和贴图；其相邻端点解析尚未接入世界扫描。原版 `Container` 适配层已支持普通可堆叠 `ItemStack`，并将无法无损重建的自定义数据、非堆叠物品拒绝在物流系统外；它尚未接入玩家/原版容器的持久化回执。玩家交互存取、可放置 funnel 与其端点解析、可见 belt 仍未完成；当前 depot 会拒绝被破坏，直到库存与活动事务均为空。
